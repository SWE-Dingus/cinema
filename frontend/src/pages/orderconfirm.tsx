  ...
  return (
    <div className="min-h-screen p-5 bg-[#1b0c1a] text-white">
      ...
      <div className="bg-[#2a1c2a] p-6 rounded-lg shadow-md mb-8">
        <h2 className="text-2xl font-bold mb-4 text-[#fadcd5]">Movie: {orderDetails.movieTitle}</h2>
        <p className="text-lg">Showtime: <span className="font-semibold">{orderDetails.showtime}</span></p>
        <p className="text-lg">
          Seats: <span className="font-semibold">{orderDetails.seatNumbers.length > 0 ? orderDetails.seatNumbers.join(', ') : 'No seats selected'}</span>
        </p>
        <p className="mt-4 text-lg font-bold text-[#fadcd5]">Total Price: $X.XX</p>
      </div>
